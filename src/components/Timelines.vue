<style scoped>
.btn {
  @apply active:scale-95 active:ring-gray-200 active:ring duration-300 rounded-3xl px-3 py-2 text-gray-300 bg-gray-800 inline-block w-10/12 mt-4;
}
</style>

<template>
  <section class="w-full bg-gradient-to-b from-customBrown to-customGreen">
    <section class="container-section pb-12">
      <HeaderSection text-color="text-gray-100"
        subtitle="Dengan tidak mengurangi rasa hormat, kami mengajak para tamu undangan agar bisa berhadir pada serangkaian acara kami"
        title="Runtutan Acara"></HeaderSection>
      <Card>
        <template v-slot:body>
          <div class="px-6 py-8 text-center text-sm font-medium" v-for="(timeline, index) in timelines" :key="index"
            style="background-color: #366975;">
            <section data-aos="fade-up">
              <h1 class="text-xl satisfy-font" style="color: #123133; font-weight: bold;">{{ timeline.title }}</h1>
              ...
              <p class="text-gray-200">{{ timeline.location }}</p>
              <p class="text-gray-200">{{ timeline.address }}</p>
              <a class="btn" href="https://maps.app.goo.gl/rJMSBN28UMwFM52R6" target="_blank" rel="noopener noreferrer">
